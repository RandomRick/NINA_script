...
function MainFunc () {
    Try
    {
        ...
        Write-Output ("System id handle is " + $system_id)
...
#        WriteRead ("AT+UBTGCHA=bcbacc6c2c394d40ae3fc158c7216ec8,10,1,1")        # Device Set Value
        ...
        WriteRead ("AT+UBTGCHA=2A19,10,1,1")                                    # Battery Level
                        ...
                        SetupToRead("AT+UBTGRR=0,29")
                        SetupToRead("AT+UBTGRR=0,34")
                        Start-Sleep -Milliseconds 1000
                        for($i=11; $i -le 200; $i++)
                        {
                                               
                                               WriteRead("AT+UBTGSN=0,24,$i")
                                               Start-Sleep -Milliseconds 1000
                                               WriteRead("AT+UBTGSN=0,29,$i")
                                               Start-Sleep -Milliseconds 1000
                                               WriteRead("AT+UBTGSN=0,34,Afff")
...
function WriteRead ([string] $stringIn)
{
    ...
    do
    {
        ...
        Write ("IN:  " + $Answer)
        # if declaring a characteristics, get the handle that's returned
...
function SetupToRead($stringSet)
{
            "`r"
    Write ("Transaction " + $vars::transactioncount)
    $vars::transactioncount++
            Write ("OUT: " + $stringSet)
            $port.Write($stringSet + "`r")
            do{
            $setUp = $port.ReadLine()
            Write ("IN:  " + $setUp)
    } until ($setUp -like "+UUBTGRR:*")
